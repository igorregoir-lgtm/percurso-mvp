...
> 422 para a turma dela, por construção (`src/api.js:424`). Não dava para trocar a persona e manter
...
> psicóloga não pode registrar o consentimento — só a coordenação (`src/api.js:684`), e a tela diz
> isso a ela (`public/app.js:1418`). O sucesso da tarefa é ela entender **por que não sai**, não
