...
deste documento — nem por nenhum dos 177 testes unitários ou das 431 asserções de fluxo do
repositório.
...
> que torna a agenda do ciclo e a observação por criança inexecutáveis para ela (`src/api.js:421`).
...
(`public/app.js:4965`); desde 04/09/2026 o relato **abre sozinho** na Vivência, logo depois de confirmar a folha. O limiar de 20 s existe para separar "ela não
...
sem culpa em `#/hoje`. A retomada lê a tabela `atividade` (`src/domain.js:1036`), não os encontros:
