...
deste documento — nem por nenhum dos 192 testes unitários ou das 471 asserções de fluxo do
repositório.
...
> que torna a agenda do ciclo e a observação por criança inexecutáveis para ela (`src/api.js:424`).
...
(`public/app.js:5521`); desde 04/09/2026 o relato **abre sozinho** na Vivência, logo depois de confirmar a folha. O limiar de 20 s existe para separar "ela não
...
sem culpa em `#/hoje`. A retomada lê a tabela `atividade` (`src/domain.js:1040`), não os encontros:
