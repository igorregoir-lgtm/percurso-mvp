...
Saída da última execução: [`EVIDENCIAS-DE-TESTE.txt`](EVIDENCIAS-DE-TESTE.txt) — **273 passaram,
0 falharam**.

Há também uma bateria de **124 testes unitários** das regras críticas de domínio (filtro de
...
síntese, fecho de ciclo, cadastro de pessoas), que roda sem servidor, contra um banco temporário descartável:
