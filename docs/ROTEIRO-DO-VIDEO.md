...
4. **Cena 9 (recado):** abrir pelo botão **"Recado para os responsáveis"** no cartão da folha, na
   ...
   (`public/app.js:610`). Até ali ele sumia em dia não letivo e a cena tinha de abrir `#/sai-daqui?aba=recado`
...
tentar abrir a ficha de uma criança neste perfil devolve **443**.
...
```
node scripts/smoke-test.mjs   → 443 passaram · 0 falharam
node scripts/unit-test.mjs    → 179 passaram · 0 falharam
```
