...
| A captura por voz é mais barata que o formulário | Taxa de correção pós-extração; limite declarado: acima de 40%, o extrator está pior que o formulário e a decisão deve ser revista | Instrumentada em `#/painel?aba=scores` (decisão 13); tarefa 3 do protocolo de validação | **pendente** — sem sessão com usuária real |
| O registro acontece de fato, ciclo após ciclo | Cobertura do registro por turma | Score de cobertura (`#/painel?aba=scores`, `#/painel`) — mede o sistema, não a professora | só dados sintéticos; sem operação real |
| A devolução (pauta de segunda) é útil, não enfeite | Taxa de aceite × descarte das sugestões | `#/hoje?detalhe=semana` — o descarte é medido por desenho | só dados sintéticos; sem operação real |
