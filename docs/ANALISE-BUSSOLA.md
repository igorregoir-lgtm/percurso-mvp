...
|---|---|---|
| *"Fluxo aceitar/dispensar da pauta com histórico"* — rejeitado como fase 2 | **Adotado.** O pack o coloca como F11 e mostra por quê: o **descarte** é a métrica que diz se a sugestão está genérica. Sem ele, não há como saber se a pauta está sendo lida. | tabela `pauta`, `#/hoje?detalhe=semana`, `taxaDeDescarte` |
...
| *"Resumo falado transcrito por LLM na nuvem"* — rejeitado por sair da organização | **A rejeição continua de pé, e a alternativa cresceu.** A dica de ditado pelo teclado virou uma tela de captura por voz com `SpeechRecognition` do próprio navegador (sem teto de tempo desde 03/09/2026) e, para a fala longa, o whisper.cpp no computador do Instituto: o áudio nunca chega ao servidor do Percurso (a transcrição é do navegador — no aparelho quando ele sabe fazê-la, no serviço do fornecedor quando não sabe), e existe o agente extrator determinístico do lado do servidor. | `#/registrar`, `src/voz.js`, decisão técnica nº 13 |
