...
export const THREADS = Math.max(1, Math.min(8, (process.env.PERCURSO_AUDIO_THREADS | 0) || 4));

/**
 * A VELOCIDADE OBSERVADA, e nao estimada (OPAR 05/09/2026).
 *
 * `fator` e' quantos segundos de processamento cada segundo de audio custa
 * NESTA maquina: 0,5 significa duas vezes mais rapido que o tempo real; 6
 * significa que dez minutos de narracao levam uma hora. A divida declarada
 * dizia que "~6x tempo real" nao diz a direcao — agora o produto diz, medindo.
 *
 * Mediana, nao media: uma transcricao que bateu no timeout distorceria a media
 * e faria a tela prometer o que a maquina nao faz.
 */
export function velocidadeObservada() {
  const linhas = all(`SELECT ms, audio_s FROM transcricao_medida ORDER BY id DESC LIMIT 50`);
  if (!linhas.length) return { medicoes: 0, fator_mediano: null, segundos_por_minuto: null };
  const fatores = linhas.map(l => l.ms / 1000 / l.audio_s).sort((a, b) => a - b);
  const meio = Math.floor(fatores.length / 2);
  const mediana = fatores.length % 2 ? fatores[meio] : (fatores[meio - 1] + fatores[meio]) / 2;
  return {
    medicoes: linhas.length,
    fator_mediano: Math.round(mediana * 100) / 100,
    // O numero que a tela usa: quanto custa transcrever um minuto de fala.
    segundos_por_minuto: Math.round(mediana * 60),
  };
}

/** Guarda a medicao. Sem texto, sem pessoa, sem encontro — so' a maquina. */
export function registrarMedicao({ audio_s, ms }) {
  if (!Number.isFinite(audio_s) || !Number.isFinite(ms) || audio_s <= 0) return null;
  run(`INSERT INTO transcricao_medida (audio_s, ms, modelo, threads, criado_em) VALUES (?,?,?,?,?)`,
    Math.round(audio_s), Math.round(ms), MODELO.split('/').pop(), THREADS, new Date().toISOString());
  return true;
}
...
export function estadoDoAudio() {
  return {
    ...
    pendentes: pendentes(),
    velocidade: velocidadeObservada(),
  };
}
...
export async function transcrever(buffer, { idioma = 'pt' } = {}) {
  ...
  try {
    ...
    // WAV 16 kHz mono 16-bit: 32.000 bytes por segundo, menos o cabecalho de 44.
    // E' o formato que o cliente sempre manda (`paraWav16k`), entao a duracao e'
    // derivavel sem perguntar nada a ninguem.
    const audioS = Math.max(1, Math.round((buffer.length - 44) / 32000));
    return { texto, bytes: buffer.length, audio_s: audioS, ms, fator: Math.round((ms / 1000 / audioS) * 100) / 100 };
  } catch (e) {
    if (e.status) throw e;
