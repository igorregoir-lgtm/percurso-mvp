...
// 6 · Aurora (assistente) com modelo: schema, scrub da fala, ação validada ----
{
  ...
  T('Aurora com modelo: origem modelo e forma {resposta, fala, acao}',
    r.origem === 'modelo' && typeof r.resposta === 'string' && 'fala' in r && 'acao' in r);
  T('Aurora: ação devolvida pelo modelo é do catálogo do papel',
  ...
  T('Aurora: fala com pseudônimo é DESCARTADA pelo scrub do servidor',
  ...
  T('Aurora: fala:null deliberada do modelo FICA nula (não vira a resposta de tela)',
  ...
  T('Aurora: diretoria + nome de criança = recusa determinística, sem modelo',
  ...
  T('Aurora: timeout do modelo cai no guia (nunca 503)', r5.origem === 'guia' && !!r5.resposta);
}
